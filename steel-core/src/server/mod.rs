...
use steel_protocol::packets::game::{
    CEntityEvent, CGameEvent, CLogin, CPlayerInfoUpdate, CRemovePlayerInfo, CSystemChat, CTabList,
    CTickingState, CTickingStep, CommonPlayerSpawnInfo, GameEventType, RelativeMovement,
};
...
mod tests {
    ...
    impl TestEntity {
        fn new(entity_type: EntityTypeRef, projectile_owner_uuid: Option<Uuid>) -> SharedEntity {
    ...
    fn ender_pearl_end_return_requires_owner_seen_credits_when_owner_is_player() {
        ...
        assert!(
            !blocked_pearl.with_entity(|entity| can_entity_return_from_end_to_overworld(
                entity,
                owner_seen_credits
            ))
        );
        assert!(
            allowed_pearl.with_entity(|entity| can_entity_return_from_end_to_overworld(
                entity,
                owner_seen_credits
            ))
        );
        assert!(
            unknown_owner_pearl.with_entity(|entity| can_entity_return_from_end_to_overworld(
                entity,
                owner_seen_credits
            ))
        );
        assert!(no_player_owner_pearl.with_entity(|entity| {
            can_entity_return_from_end_to_overworld(entity, owner_seen_credits)
        }));
        assert!(
            item.with_entity(|entity| can_entity_return_from_end_to_overworld(
                entity,
                owner_seen_credits
            ))
        );
...
fn direct_passengers_allow_end_return(entity: &dyn Entity) -> bool {
    for passenger in entity.passengers() {
        let blocks_return = passenger.with_entity(|passenger| {
            passenger
                .as_player()
...
pub(crate) enum PlayerResetRequest {
    ...
    /// Detach from the world for the vanilla End credits screen.
    EndCredits(Arc<ServerPlayer>),
}
...
impl ServerJob for NetherPortalTeleportJob {
    fn poll(&mut self, context: &mut ServerJobContext) -> JobPoll {
        ...
        if !is_allowed_to_enter_portal(&self.source_world, &self.target_world)
            || !self.entity.with_entity(|entity| {
                server.can_teleport_between_worlds(entity, &self.source_world, &self.target_world)
            })
        {
            self.clear_pending();
            return JobPoll::Finished;
        }
        let Some(transition) = self.entity.with_entity(|entity| {
            nether_portal::calculate_transition(
                &self.source_world,
                &self.target_world,
                entity,
                self.portal_pos,
        ...
        loop {
            match &mut self.phase {
                EndPortalTeleportPhase::EntryToEnd {
                    ...
                    let Some(transition) = entity.with_entity(|entity| {
                        end_portal::calculate_entry_transition(target_world, entity)
                    }) else {
                        clear_pending_world_change(&entity, pending_token);
                        return JobPoll::Finished;
                    };
                ...
                EndPortalTeleportPhase::ReturningEntity {
                    ...
                    let transition = entity.with_entity(|entity| {
                        end_portal::calculate_entity_return_transition(
                            target_world,
                            entity,
                            respawn_data,
                        )
                ...
                EndPortalTeleportPhase::LoadingPlayerRespawn {
                    ...
                    let transition = entity.with_entity(|entity| {
                        end_portal::calculate_player_return_transition(
                            target_world,
                            entity,
                            spawn.position,
                            spawn.rotation,
            ...
            match &mut self.phase {
                EndGatewayTeleportPhase::LoadingReady { request } => match request.poll() {
                    ...
                    ChunkRequestState::Ready => {
                        ...
                        let Some(transition) = entity.with_entity(|entity| {
                            end_gateway::calculate_transition(
                                &source_world,
                                entity,
                                portal_pos,
                                source_is_end,
...
impl EnderPearlRestoreJob {
    fn new(player: Arc<ServerPlayer>, world: Arc<World>, entity: PersistentEntity) -> Option<Self> {
            ...
            ChunkRequestState::Ready => {
                ...
                if !restore_ender_pearl_for_player(&self.player, &self.world, &self.entity) {
                    self.player
                        .entity
                        .lock()
                        .remove_pending_ender_pearl(self.uuid);
                }
                JobPoll::Finished
            }
        }
...
fn restore_ender_pearl_for_player(
    ...
    let owner: SharedEntity = player.entity.lock().shared_entity();
    for entity in &entities {
        entity.with_entity(|entity| entity.restore_owner_reference(&owner));
    }
    ...
    player.entity.lock().register_ender_pearl(&pearl);
...
impl Server {
    ...
    pub fn queue_player_join(self: &Arc<Self>, player: Arc<ServerPlayer>) {
        ...
        if !self.reserve_player_join(&player) {
            player
                .entity
                .lock()
                .disconnect("You are already connected to this server");
            return;
        }
    ...
    fn reserve_player_join(&self, player: &ServerPlayer) -> bool {
        let uuid = player.uuid;
    ...
    fn admit_reserved_player(&self, player: Arc<ServerPlayer>) -> bool {
        let uuid = player.uuid;
        let mut admissions = self.player_admissions.lock();
    ...
    fn reserve_player_disconnect(&self, player: &Arc<ServerPlayer>) -> bool {
        let uuid = player.uuid;
        let mut admissions = self.player_admissions.lock();
        if admissions.contains_key(&uuid) {
            return false;
    ...
    fn remove_online_player_sync(&self, player: &Arc<ServerPlayer>) {
    ...
    fn broadcast_to_online_with<P: ClientPacket, F: Fn(&Player) -> P>(&self, packet: F) {
        self.online_players.iter_players(|_, session| {
            let player = session.entity.lock();
    ...
    fn schedule_ender_pearl_restores(&self, player: &Arc<ServerPlayer>, state: &DomainPlayerState) {
        let pearls = Self::ender_pearls_to_restore(state);
        if pearls.is_empty() {
            player.entity.lock().clear_pending_ender_pearls();
            return;
        }
        player
            .entity
            .lock()
            .set_pending_ender_pearls(pearls.clone());
        for pearl in pearls {
            let pearl_uuid = Uuid::from_bytes(pearl.entity.uuid);
            let resolved = self.resolve_pearl_world(&pearl.world, &player.entity.lock());
            let Some(world) = resolved else {
                player.entity.lock().remove_pending_ender_pearl(pearl_uuid);
                continue;
            };
    ...
    fn ender_pearls_to_restore(state: &DomainPlayerState) -> Vec<PersistentEnderPearl> {
        match &state.data {
            DomainPlayerData::Saved { data, .. } => data.ender_pearls.clone(),
    ...
    fn send_chunks_for_player(
        ...
        let Some(view) = *shared.last_tracking_view.lock() else {
            return;
        };

        // Entities in the chunks we just sent must be spawned for this player even if
        // they never move. Snapshot the sent set and release the chunk-sender lock
        // *before* taking the entity lock: the game tick locks the entity and then the
        // chunk sender, so acquiring them in that order here would close a cycle.
        let sent_chunks = shared.chunk_sender.lock().sent_chunks_snapshot();
        let player = shared.entity.lock();
        world
            .entity_tracker()
    ...
    fn queue_nether_portal_change(
        ...
        if !is_allowed_to_enter_portal(&source_world, &target_world)
            || !entity.with_entity(|entity| {
                self.can_teleport_between_worlds(entity, &source_world, &target_world)
            })
        {
            clear_pending_world_change(&entity, pending_token);
            return;
    ...
    fn process_player_resets(&self) {
        ...
        for request in resets {
            match request {
                ...
                PlayerResetRequest::EndCredits(player) => {
                    let world = player.entity.lock().get_world();
        ...
        self.worlds
            ...
            .find_map(|world| {
                world.get_entity_by_uuid(uuid).and_then(|entity| {
                    entity.with_entity(|entity| {
                        entity.as_player().map(|player| player.has_seen_credits())
                    })
                })
            })
    }
