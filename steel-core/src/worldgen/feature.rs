...
use steel_worldgen::math::{floor, lerp};
...
impl FeatureDecorationRunner {
    ...
    fn place_configured_feature(
        ...
        match Self::configured_feature_kind(registry, feature) {
            ...
            ConfiguredFeatureKind::Ore(config) => {
                Self::place_ore_feature(region, registry, random, config, origin)
            }
            ConfiguredFeatureKind::ScatteredOre(config) => {
                Self::place_scattered_ore_feature(region, registry, random, config, origin)
            }
            _ => {
    ...
    fn place_scattered_ore_feature(
        region: &mut WorldGenRegion<'_>,
        registry: &Registry,
        random: &mut Xoroshiro,
        config: &OreConfiguration,
    ...
    fn random_scattered_ore_offset(random: &mut Xoroshiro, max_distance: i32) -> i32 {
    ...
    fn rule_test_matches(registry: &Registry, target: &RuleTest, state: BlockStateId) -> bool {
    ...
    fn should_skip_air_check(random: &mut Xoroshiro, discard_chance_on_air_exposure: f32) -> bool {
    ...
    fn is_adjacent_to_air(region: &WorldGenRegion<'_>, registry: &Registry, pos: BlockPos) -> bool {
    ...
    fn is_air_block_state(registry: &Registry, state: BlockStateId) -> bool {
        let Some(block) = registry.blocks.by_state_id(state) else {
    ...
    fn can_write_to_pos(region: &WorldGenRegion<'_>, pos: BlockPos) -> bool {
