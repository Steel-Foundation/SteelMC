...
fn apply_mirror(x: i32, z: i32, mirror: Mirror) -> (i32, i32) {
    match mirror {
        Mirror::None => (x, z),
...
fn relative(pos: (i32, i32, i32), rotation: Rotation, dir: Direction, amount: i32) -> (i32, i32, i32) {
    let rotated = rotation.rotate(dir);
    let (dx, dy, dz) = rotated.offset();
    (pos.0 + dx * amount, pos.1 + dy * amount, pos.2 + dz * amount)
}

fn above(pos: (i32, i32, i32), amount: i32) -> (i32, i32, i32) {
    (pos.0, pos.1 + amount, pos.2)
}

/// Vanilla's `getZeroPositionWithTransform(zeroPos, Mirror.NONE, rotation, 7, 7)`
fn zero_pos_transform(
    zero: (i32, i32, i32),
    rotation: Rotation,
    size_x: i32,
    size_z: i32,
...
fn compose_rotation(base: Rotation, add: Rotation) -> Rotation {
    base.then(add)
}

/// Vanilla's `Direction.from2DDataValue`: 0=S, 1=W, 2=N, 3=E
fn dir_from_2d(value: i32) -> Direction {
    match value & 3 {
        0 => Direction::South,
        1 => Direction::West,
        2 => Direction::North,
...
impl MansionGrid {
    fn new(rng: &mut LegacyRandom) -> Self {
        ...
        Self::recursive_corridor(&mut base, rng, entrance_x, entrance_y - 2, Direction::West, 6);
        Self::recursive_corridor(&mut base, rng, entrance_x, entrance_y + 3, Direction::West, 6);
        Self::recursive_corridor(&mut base, rng, entrance_x - 2, entrance_y - 1, Direction::West, 3);
        Self::recursive_corridor(&mut base, rng, entrance_x - 2, entrance_y + 2, Direction::West, 3);
        ...
        floor_rooms[0].set_range(entrance_x + 1, entrance_y, entrance_x + 1, entrance_y + 1, ROOM_CORRIDOR_FLAG);
        floor_rooms[1].set_range(entrance_x + 1, entrance_y, entrance_x + 1, entrance_y + 1, ROOM_CORRIDOR_FLAG);
    ...
    fn setup_third_floor(
        ...
        if potential_dirs.is_empty() {
            ...
            let corridor_dir = potential_dirs[rng.next_i32_bounded(potential_dirs.len() as i32) as usize];
            Self::recursive_corridor(
                third,
                rng,
    ...
    fn get_1x2_room_direction(&self, _grid: &SimpleGrid, x: i32, y: i32, floor: usize, room_id: i32) -> Option<Direction> {
        for dir in &[Direction::North, Direction::East, Direction::South, Direction::West] {
...
pub fn generate_mansion_pieces(
    ...
    traverse_outer_walls(
        &mut pieces, &mut data, &mansion.base_grid, Direction::South,
        start_x, start_y, end_x, end_y,
    );
    // Floor 1 outer walls
    traverse_outer_walls(
        &mut pieces, &mut second, &mansion.base_grid, Direction::South,
        start_x, start_y, end_x, end_y,
    );
    ...
    for y in 0..mansion.third_floor_grid.height {
        ...
        for x in (0..mansion.third_floor_grid.width).rev() {
            ...
            if is_house(&mansion.third_floor_grid, x, y) {
                third_data.position = relative(third_data.position, rotation, Direction::South, 8 + (y - start_y) * 8);
                third_data.position = relative(third_data.position, rotation, Direction::East, (x - start_x) * 8);
                traverse_wall_piece(&mut pieces, &mut third_data);
                traverse_outer_walls(
                    &mut pieces, &mut third_data, &mansion.third_floor_grid, Direction::South,
                    x, y, x, y,
                );
                done = true;
            }
    ...
    // Roofs
    create_roof(&mut pieces, above(origin, 16), rotation, &mansion.base_grid, Some(&mansion.third_floor_grid), start_x, start_y);
    create_roof(&mut pieces, above(origin, 27), rotation, &mansion.third_floor_grid, None, start_x, start_y);

    // Interior: corridors, walls, doors, rooms for 3 floors
    for floor_num in 0..3_usize {
        let floor_origin = above(origin, 8 * floor_num as i32 + if floor_num == 2 { 3 } else { 0 });
        let rooms = &mansion.floor_rooms[floor_num];
        let grid = if floor_num == 2 { &mansion.third_floor_grid } else { &mansion.base_grid };
        let south_piece = if floor_num == 0 { "carpet_south_1" } else { "carpet_south_2" };
        let west_piece = if floor_num == 0 { "carpet_west_1" } else { "carpet_west_2" };

        // Corridors
        for y in 0..grid.height {
            for x in 0..grid.width {
                if grid.get(x, y) == 1 {
                    let mut pos = relative(floor_origin, rotation, Direction::South, 8 + (y - start_y) * 8);
                    pos = relative(pos, rotation, Direction::East, (x - start_x) * 8);
                    pieces.push(piece_bb(pos, template_size("corridor_floor"), rotation, Mirror::None));

                    if grid.get(x, y - 1) == 1 || (rooms.get(x, y - 1) & ROOM_CORRIDOR_FLAG) != 0 {
                        let p = above(relative(relative(pos, rotation, Direction::East, 1), rotation, Direction::South, 0), 1);
                        pieces.push(piece_bb(p, template_size("carpet_north"), rotation, Mirror::None));
                    }
                    if grid.get(x + 1, y) == 1 || (rooms.get(x + 1, y) & ROOM_CORRIDOR_FLAG) != 0 {
                        let p = above(relative(relative(pos, rotation, Direction::South, 1), rotation, Direction::East, 5), 1);
                        pieces.push(piece_bb(p, template_size("carpet_east"), rotation, Mirror::None));
                    }
                    if grid.get(x, y + 1) == 1 || (rooms.get(x, y + 1) & ROOM_CORRIDOR_FLAG) != 0 {
                        let p = relative(relative(pos, rotation, Direction::South, 5), rotation, Direction::West, 1);
                        pieces.push(piece_bb(p, template_size(south_piece), rotation, Mirror::None));
                    }
                    if grid.get(x - 1, y) == 1 || (rooms.get(x - 1, y) & ROOM_CORRIDOR_FLAG) != 0 {
                        let p = relative(relative(pos, rotation, Direction::West, 1), rotation, Direction::North, 1);
                        pieces.push(piece_bb(p, template_size(west_piece), rotation, Mirror::None));
                    }
                }
            }
        }
        ...
        let wall_piece = if floor_num == 0 { "indoors_wall_1" } else { "indoors_wall_2" };
        let door_piece = if floor_num == 0 { "indoors_door_1" } else { "indoors_door_2" };
        ...
        for y in 0..grid.height {
            for x in 0..grid.width {
                ...
                let mut room_pos = relative(floor_origin, rotation, Direction::South, 8 + (y - start_y) * 8);
                ...
                if is_house(grid, x - 1, y) && !mansion.is_room_id(x - 1, y, floor_num, room_id) {
                    let template = if door_dir == Some(Direction::West) { door_piece } else { wall_piece };
                    pieces.push(piece_bb(room_pos, template_size(template), rotation, Mirror::None));
                }

                // East wall (corridor side)
                if grid.get(x + 1, y) == 1 && !is_corridor_start {
                    let p = relative(room_pos, rotation, Direction::East, 8);
                    let template = if door_dir == Some(Direction::East) { door_piece } else { wall_piece };
                ...
                if is_house(grid, x, y + 1) && !mansion.is_room_id(x, y + 1, floor_num, room_id) {
                    let p = relative(relative(room_pos, rotation, Direction::South, 7), rotation, Direction::East, 7);
                    let template = if door_dir == Some(Direction::South) { door_piece } else { wall_piece };
                    pieces.push(piece_bb(p, template_size(template), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                }

                // North wall (corridor side)
                if grid.get(x, y - 1) == 1 && !is_corridor_start {
                    let p = relative(relative(room_pos, rotation, Direction::North, 1), rotation, Direction::East, 7);
                    let template = if door_dir == Some(Direction::North) { door_piece } else { wall_piece };
                    pieces.push(piece_bb(p, template_size(template), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                }
                ...
                if room_type == ROOM_1X1 {
                    ...
                    if let Some(rd) = room_dir {
                        add_room_1x2(&mut pieces, room_pos, rotation, rd, door_dir.unwrap(), floor_num, is_stairs, rng);
                    }
                } else if room_type == ROOM_2X2 && door_dir.is_some() && door_dir != Some(Direction::Up) {
                    let dd = door_dir.unwrap();
                    let mut room_dir = dd.rotate_y_clockwise();
                    if !mansion.is_room_id(x + room_dir.offset().0, y + room_dir.offset().2, floor_num, room_id) {
                        room_dir = room_dir.opposite();
                    }
                    add_room_2x2(&mut pieces, room_pos, rotation, room_dir, dd, floor_num, rng);
...
fn place_entrance(pieces: &mut Vec<BoundingBox>, data: &mut PlacementData) {
    ...
    pieces.push(piece_bb(pos, template_size("entrance"), data.rotation, Mirror::None));
...
fn traverse_wall_piece(pieces: &mut Vec<BoundingBox>, data: &mut PlacementData) {
    ...
    pieces.push(piece_bb(pos, template_size(data.wall_type), data.rotation, Mirror::None));
...
fn traverse_turn(pieces: &mut Vec<BoundingBox>, data: &mut PlacementData) {
    ...
    pieces.push(piece_bb(data.position, template_size("wall_corner"), data.rotation, Mirror::None));
...
fn traverse_inner_turn(_pieces: &mut Vec<BoundingBox>, data: &mut PlacementData) {
...
fn traverse_outer_walls(
    ...
    loop {
        ...
        if !is_house(grid, grid_x + dx, grid_y + dz) {
            ...
            && is_house(grid, grid_x + dx + dir.rotate_y_counter_clockwise().offset().0, grid_y + dz + dir.rotate_y_counter_clockwise().offset().2)
        {
...
fn create_roof(
    ...
    for y in 0..grid.height {
        for x in 0..grid.width {
            let mut pos = relative(roof_origin, rotation, Direction::South, 8 + (y - start_y) * 8);
            pos = relative(pos, rotation, Direction::East, (x - start_x) * 8);
            let is_above = above_grid.is_some_and(|g| is_house(g, x, y));

            if is_house(grid, x, y) && !is_above {
                pieces.push(piece_bb(above(pos, 3), template_size("roof"), rotation, Mirror::None));

                if !is_house(grid, x + 1, y) {
                    let p = relative(pos, rotation, Direction::East, 6);
                    pieces.push(piece_bb(p, template_size("roof_front"), rotation, Mirror::None));
                }
                if !is_house(grid, x - 1, y) {
                    let p = relative(relative(pos, rotation, Direction::East, 0), rotation, Direction::South, 7);
                    pieces.push(piece_bb(p, template_size("roof_front"), compose_rotation(rotation, Rotation::Clockwise180), Mirror::None));
                }
                if !is_house(grid, x, y - 1) {
                    let p = relative(pos, rotation, Direction::West, 1);
                    pieces.push(piece_bb(p, template_size("roof_front"), compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None));
                }
                if !is_house(grid, x, y + 1) {
                    let p = relative(relative(pos, rotation, Direction::East, 6), rotation, Direction::South, 6);
                    pieces.push(piece_bb(p, template_size("roof_front"), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                }
            }
        }
    }
        ...
        for y in 0..grid.height {
            for x in 0..grid.width {
                ...
                if !is_house(grid, x + 1, y) {
                    ...
                    pieces.push(piece_bb(p, template_size("small_wall"), rotation, Mirror::None));
                }
                if !is_house(grid, x - 1, y) {
                    let p = relative(relative(pos, rotation, Direction::West, 1), rotation, Direction::South, 6);
                    pieces.push(piece_bb(p, template_size("small_wall"), compose_rotation(rotation, Rotation::Clockwise180), Mirror::None));
                }
                if !is_house(grid, x, y - 1) {
                    let p = relative(relative(pos, rotation, Direction::West, 0), rotation, Direction::North, 1);
                    pieces.push(piece_bb(p, template_size("small_wall"), compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None));
                }
                if !is_house(grid, x, y + 1) {
                    let p = relative(relative(pos, rotation, Direction::East, 6), rotation, Direction::South, 7);
                    pieces.push(piece_bb(p, template_size("small_wall"), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                }

                // Corners
                if !is_house(grid, x + 1, y) {
                    if !is_house(grid, x, y - 1) {
                        let p = relative(relative(pos, rotation, Direction::East, 7), rotation, Direction::North, 2);
                        pieces.push(piece_bb(p, template_size("small_wall_corner"), rotation, Mirror::None));
                    }
                    if !is_house(grid, x, y + 1) {
                        let p = relative(relative(pos, rotation, Direction::East, 8), rotation, Direction::South, 7);
                        pieces.push(piece_bb(p, template_size("small_wall_corner"), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                    }
                }
                if !is_house(grid, x - 1, y) {
                    if !is_house(grid, x, y - 1) {
                        let p = relative(relative(pos, rotation, Direction::West, 2), rotation, Direction::North, 1);
                        pieces.push(piece_bb(p, template_size("small_wall_corner"), compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None));
                    }
                    if !is_house(grid, x, y + 1) {
                        let p = relative(relative(pos, rotation, Direction::West, 1), rotation, Direction::South, 8);
                        pieces.push(piece_bb(p, template_size("small_wall_corner"), compose_rotation(rotation, Rotation::Clockwise180), Mirror::None));
                    }
                }
            }
        }
        ...
        for x in 0..grid.width {
            ...
            if !is_house(grid, x + 1, y) {
                ...
                if !is_house(grid, x, y + 1) {
                    ...
                    pieces.push(piece_bb(p2, template_size("roof_corner"), rotation, Mirror::None));
                } else if is_house(grid, x + 1, y + 1) {
                    let p2 = relative(p, rotation, Direction::South, 5);
                    pieces.push(piece_bb(p2, template_size("roof_inner_corner"), rotation, Mirror::None));
                }
                if !is_house(grid, x, y - 1) {
                    pieces.push(piece_bb(p, template_size("roof_corner"), compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None));
                } else if is_house(grid, x + 1, y - 1) {
                    let p2 = relative(relative(pos, rotation, Direction::East, 9), rotation, Direction::North, 2);
                    pieces.push(piece_bb(p2, template_size("roof_inner_corner"), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                }
            }
            ...
            if !is_house(grid, x - 1, y) {
                ...
                if !is_house(grid, x, y + 1) {
                    let p2 = relative(p, rotation, Direction::South, 6);
                    pieces.push(piece_bb(p2, template_size("roof_corner"), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None));
                } else if is_house(grid, x - 1, y + 1) {
                    let p2 = relative(relative(p, rotation, Direction::South, 8), rotation, Direction::West, 3);
                    pieces.push(piece_bb(p2, template_size("roof_inner_corner"), compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None));
                }
                if !is_house(grid, x, y - 1) {
                    pieces.push(piece_bb(p, template_size("roof_corner"), compose_rotation(rotation, Rotation::Clockwise180), Mirror::None));
                } else if is_house(grid, x - 1, y - 1) {
                    let p2 = relative(p, rotation, Direction::South, 1);
                    pieces.push(piece_bb(p2, template_size("roof_inner_corner"), compose_rotation(rotation, Rotation::Clockwise180), Mirror::None));
                }
            }
        }
    }
...
fn add_room_1x1(
    ...
    match door_dir {
        ...
        Some(Direction::North) => { piece_rot = Rotation::CounterClockwise90; kind = "1x1"; }
        Some(Direction::West) => { piece_rot = Rotation::Clockwise180; kind = "1x1"; }
        Some(Direction::South) => { piece_rot = Rotation::Clockwise90; kind = "1x1"; }
        _ => kind = "1x1s",
    }
...
fn add_room_1x2(
    ...
    let (pos, rot, mirror, kind) = match (door_dir, room_dir) {
        (Direction::East, Direction::South) => {
            (relative(room_pos, rotation, Direction::East, 1), rotation, Mirror::None, "1x2side")
        }
        (Direction::East, Direction::North) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 1), rotation, Direction::South, 6);
            (p, rotation, Mirror::LeftRight, "1x2side")
        }
        (Direction::West, Direction::North) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 7), rotation, Direction::South, 6);
            (p, compose_rotation(rotation, Rotation::Clockwise180), Mirror::None, "1x2side")
        }
        (Direction::West, Direction::South) => {
            (relative(room_pos, rotation, Direction::East, 7), rotation, Mirror::FrontBack, "1x2side")
        }
        (Direction::South, Direction::East) => {
            (relative(room_pos, rotation, Direction::East, 1), compose_rotation(rotation, Rotation::Clockwise90), Mirror::LeftRight, "1x2side")
        }
        (Direction::South, Direction::West) => {
            (relative(room_pos, rotation, Direction::East, 7), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None, "1x2side")
        }
        (Direction::North, Direction::West) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 7), rotation, Direction::South, 6);
            (p, compose_rotation(rotation, Rotation::Clockwise90), Mirror::FrontBack, "1x2side")
        }
        (Direction::North, Direction::East) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 1), rotation, Direction::South, 6);
            (p, compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None, "1x2side")
        }
        (Direction::South, Direction::North) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 1), rotation, Direction::North, 8);
            (p, rotation, Mirror::None, "1x2front")
        }
        (Direction::North, Direction::South) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 7), rotation, Direction::South, 14);
            (p, compose_rotation(rotation, Rotation::Clockwise180), Mirror::None, "1x2front")
        }
        (Direction::West, Direction::East) => {
            (relative(room_pos, rotation, Direction::East, 15), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None, "1x2front")
        }
        (Direction::East, Direction::West) => {
            let p = relative(relative(room_pos, rotation, Direction::West, 7), rotation, Direction::South, 6);
            (p, compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None, "1x2front")
        }
        (Direction::Up, Direction::East) => {
            (relative(room_pos, rotation, Direction::East, 15), compose_rotation(rotation, Rotation::Clockwise90), Mirror::None, "1x2secret")
        }
        (Direction::Up, Direction::South) => {
            let p = relative(relative(room_pos, rotation, Direction::East, 1), rotation, Direction::North, 0);
...
fn add_room_2x2(
    ...
    let (east, south, rot, mirror) = match (door_dir, room_dir) {
        ...
        (Direction::North, Direction::East) => (1, 14, compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::None),
        (Direction::North, Direction::West) => (7, 14, compose_rotation(rotation, Rotation::CounterClockwise90), Mirror::LeftRight),
        (Direction::South, Direction::West) => (7, -8, compose_rotation(rotation, Rotation::Clockwise90), Mirror::None),
        (Direction::South, Direction::East) => (1, -8, compose_rotation(rotation, Rotation::Clockwise90), Mirror::LeftRight),
        (Direction::West, Direction::North) => (15, 6, compose_rotation(rotation, Rotation::Clockwise180), Mirror::None),
        (Direction::West, Direction::South) => (15, 0, rotation, Mirror::FrontBack),
        _ => return,
    };

    let pos = relative(relative(room_pos, rotation, Direction::East, east), rotation, Direction::South, south);
...
impl<N: DimensionNoises> Structure<N> for WoodlandMansionStructure {
    fn find_generation_point(
        ...
        let bbs = generate_mansion_pieces(
            (ctx.chunk_min_x, 0, ctx.chunk_min_z),
            rotation,
            rng,
        );
        let pieces = bbs
