...
mod tests {
    ...
    fn recorded_calls(entity: &SharedEntity) -> Vec<BubbleColumnCall> {
        entity.with_entity(|entity| {
            entity
                .downcast_mut::<RecordingEntity>()
                .expect("recording entity")
                .calls()
        })
    }
    ...
    fn precise_entity_with_open_block_above_uses_above_bubble_column_hook() {
        ...
        init_behaviors();
        let level = TestLevel::default();
        let entity = RecordingEntity::shared();
    ...
    fn imprecise_entity_does_not_apply_bubble_column_effect() {
        ...
        assert_eq!(recorded_calls(&entity), Vec::new());
    }
}
