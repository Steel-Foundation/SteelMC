...
//!
//! Throwing an ender pearl spawns a [`EnderPearl`] from the player's eye,
...
use crate::entity::entities::EnderPearl;
...
impl ItemBehavior for EnderPearlItem {
    fn use_item(&self, context: &mut UseItemContext) -> InteractionResult {
        ...
        let entity: SharedEntity = EnderPearl::new(
        ...
        let owner = player.shared_entity();
        let (yaw, player_pitch) = player.rotation();
        entity.with_entity(|thrown| {
            let Some(pearl) = thrown.downcast_mut::<EnderPearl>() else {
                return;
            };
