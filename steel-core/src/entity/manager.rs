...
impl WorldEntityManager {
    ...
    fn can_tick_entity_now(&self, entity_id: i32) -> bool {
        ...
        // An entity mid-world-change (or riding one) must not tick in this world.
        if Self::has_pending_world_change_in_vehicle_chain(&entry.entity) {
            return false;
        }

        // The tick list is the single gate: players are `External` and are ticked by
        // the world game loop, not here, so they never enter it.
