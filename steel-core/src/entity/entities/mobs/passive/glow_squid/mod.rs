//! Vanilla GlowSquid entity — ported from Pumpkin (foundation-first).
...
use crate::entity::ai::goal::{LookAtPlayerGoal, RandomLookAroundGoal, RandomSwimmingGoal};
...
/// Entity behavior for the glow squid.
...
impl Entity for GlowSquidEntity {
    ...
    fn play_step_sound(&self, _pos: BlockPos, _block: BlockStateId) {
        self.play_sound(
            &steel_registry::sound_events::ENTITY_GLOW_SQUID_SQUIRT,
            0.15,
            1.0,
        );
    }
...
impl LivingEntity for GlowSquidEntity {
    ...
    fn hurt_sound(&self, _s: &DamageSource) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_GLOW_SQUID_HURT)
    }
    fn death_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_GLOW_SQUID_DEATH)
    }
...
impl Mob for GlowSquidEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_GLOW_SQUID_AMBIENT)
    }
    fn finalize_spawn(
        &self,
        world: &Arc<World>,
