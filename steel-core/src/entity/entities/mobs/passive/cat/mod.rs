//! Vanilla Cat entity — ported from Pumpkin (foundation-first).
...
impl LivingEntity for CatEntity {
    ...
    fn hurt_sound(&self, _s: &DamageSource) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_CAT_HURT)
    }
    fn death_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_CAT_DEATH)
    }
...
impl Mob for CatEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_CAT_AMBIENT)
    }
    fn finalize_spawn(
        &self,
        world: &Arc<World>,
