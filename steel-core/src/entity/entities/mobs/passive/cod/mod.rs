//! Vanilla Cod entity — ported from Pumpkin (foundation-first).
...
impl CodEntity {
    ...
    fn new_with_base(base: EntityBase, entity_type: EntityTypeRef) -> Self {
        ...
        fish_init_mob_base(&mob_base);
        {
            ...
            goal_selector.add_goal(2, AvoidEntityGoal::new(8.0, 1.6, 1.4));
...
impl Entity for CodEntity {
    ...
    fn play_step_sound(&self, _pos: BlockPos, _block: BlockStateId) { self.play_sound(&steel_registry::sound_events::ENTITY_COD_FLOP, 0.15, 1.0); }
    ...
    fn load_additional(&self, nbt: BorrowedNbtCompoundView<'_, '_>) {
        self.load_mob(nbt);
        if let Some(fb) = nbt.byte("FromBucket") {
            self.set_from_bucket(fb != 0);
        }
    }
}
...
impl LivingEntity for CodEntity {
    ...
    fn hurt_sound(&self, _s: &DamageSource) -> Option<SoundEventRef> { Some(&steel_registry::sound_events::ENTITY_COD_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&steel_registry::sound_events::ENTITY_COD_DEATH) }
    ...
    fn ai_step(&self) -> Option<MoveResult> {
        if !self.is_in_water() && self.on_ground() && self.vertical_collision() {
            ...
            self.play_sound(&steel_registry::sound_events::ENTITY_COD_FLOP, 1.0, 1.0);
        }
...
impl Mob for CodEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&steel_registry::sound_events::ENTITY_COD_AMBIENT) }
...
impl CodEntity {
    ...
    pub fn set_from_bucket(&self, _from_bucket: bool) {}
}
