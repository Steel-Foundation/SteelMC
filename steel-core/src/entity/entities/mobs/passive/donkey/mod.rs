//! Vanilla Donkey entity — ported from Pumpkin (foundation-first).
...
/// Entity behavior for the donkey.
...
impl LivingEntity for DonkeyEntity {
    ...
    fn hurt_sound(&self, _s: &DamageSource) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_DONKEY_HURT)
    }
    fn death_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_DONKEY_DEATH)
    }
...
impl Mob for DonkeyEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_DONKEY_AMBIENT)
    }
    fn finalize_spawn(
        &self,
        world: &Arc<World>,
