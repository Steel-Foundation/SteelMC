//! Vanilla Piglin entity — ported from Pumpkin (foundation-first).
...
/// Entity behavior for the piglin.
...
impl LivingEntity for PiglinEntity {
    ...
    fn hurt_sound(&self, _s: &DamageSource) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_PIGLIN_HURT)
    }
    fn death_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_PIGLIN_DEATH)
    }
...
impl Mob for PiglinEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> {
        Some(&steel_registry::sound_events::ENTITY_PIGLIN_AMBIENT)
    }
    fn finalize_spawn(
        &self,
        world: &Arc<World>,
