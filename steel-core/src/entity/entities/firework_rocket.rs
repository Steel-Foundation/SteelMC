...
use crate::entity::{
    Entity, EntityBase, EntityBaseLoad, EntityEventSource, EntityId, EntitySyncedData,
...
impl FireworkRocketEntity {
    ...
    pub fn attached_to_living(
        ...
        if let Ok(attached_id) = u32::try_from(attached_to.id().get()) {
            rocket
                .entity_data
                .lock()
                .attached_to_target
    ...
    fn attached_entity(&self, world: &Arc<World>) -> Option<SharedEntity> {
        ...
        let attached = world.get_entity_by_id(EntityId::new(attached_id))?;
...
mod tests {
    ...
    fn firework_damage_source_has_no_raw_position() {
        ...
        assert_eq!(source.direct_entity_id, Some(EntityId::new(23)));
