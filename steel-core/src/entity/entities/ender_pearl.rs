...
//!
//! Mirrors vanilla `ThrownEnderpearl` (yarn `EnderPearl`) on the Steel
...
pub struct EnderPearl {
    ...
    entity_data: EnderPearlEntityData,
...
// SAFETY: This key is owned by Steel and uniquely identifies `EnderPearl`.
unsafe impl DowncastType for EnderPearl {
...
impl EnderPearl {
    fn build(base: Weak<EntityBase>, entity_type: EntityTypeRef) -> Self {
        Self {
            base,
            entity_type,
            entity_data: EnderPearlEntityData::new(),
    ...
    /// Resolves the owner as a player, including cached cross-world refs.
    ///
    /// The player check reads the base entity type, so it does not take the owner's
    /// behavior lock.
    fn owner_player(&self) -> Option<SharedEntity> {
        let owner = self.get_owner()?;
        (owner.entity_type() == &vanilla_entities::PLAYER).then_some(owner)
    }
    ...
    fn deregister_from_owner(&self) {
        ...
        if let Some(owner) = self.owner_player() {
            owner.with_entity(|entity| {
                if let Some(player) = entity.as_player() {
                    player.deregister_ender_pearl(uuid);
                }
            });
        }
    }
    ...
    fn update_ender_pearl_ticket(&mut self, world: &Arc<World>) {
        ...
        self.ticket_timer -= 1;
        if (self.ticket_timer > 0 && !crossed_border) || self.owner_player().is_none() {
            return;
        }

        world.chunk_map.place_ender_pearl_ticket(current_chunk);
        // Vanilla `registerAndUpdateEnderPearlTicket` returns `timeout - 1`.
        self.ticket_timer = ENDER_PEARL_TICKET_TIMEOUT as i32 - 1;
    }
    ...
    fn teleport_owner(&self, world: &Arc<World>, owner: &SharedEntity, teleport_pos: DVec3) {
        // TODO: 5% endermite spawn (Endermite entity not implemented).
        let on_portal_cooldown = self.is_on_portal_cooldown();
        let portal_cooldown = owner.with_entity(|entity| {
            let Some(player) = entity.as_player() else {
                return 0;
            };
            if on_portal_cooldown {
...
impl Entity for EnderPearl {
    ...
    fn hurt(&mut self, _source: &DamageSource, _amount: f32) -> bool {
...
impl Projectile for EnderPearl {
    ...
    fn on_hit_entity(&self, entity: &SharedEntity, _location: DVec3) {
        ...
        entity.with_entity(|hit_entity| hit_entity.hurt(&damage, 0.0));
    }
    ...
    fn on_hit(&self, hit: &ProjectileHit) {
        ...
        if let Some(owner) = self.owner_player() {
            let allowed = owner.with_entity(|entity| {
                entity
                    .as_player()
                    .is_some_and(|player| Self::is_allowed_to_teleport_owner(&world, player))
            });
            if allowed {
                self.teleport_owner(&world, &owner, teleport_pos);
            }
        }
        self.deregister_from_owner();
        self.set_removed(RemovalReason::Discarded);
...
impl ThrowableProjectile for EnderPearl {}

impl ThrowableItemProjectile for EnderPearl {
...
mod tests {
    ...
    use super::EnderPearl;

    #[test]
    fn shoot_aligns_velocity_with_direction() {
        init_test_registry();

        let pearl = EnderPearl::new(
            &vanilla_entities::ENDER_PEARL,
            1,
            DVec3::ZERO,
            Weak::<World>::new(),
        );
        pearl.with_entity(|entity| {
            entity
                .downcast_mut::<EnderPearl>()
                .expect("ender pearl")
                .shoot(DVec3::new(0.0, 0.0, 1.0), 1.5, 1.0);
        });
    ...
    fn owner_round_trips() {
        ...
        pearl.with_entity(|entity| {
            let pearl = entity.downcast_mut::<EnderPearl>().expect("ender pearl");
    ...
    fn default_item_is_ender_pearl() {
        ...
        pearl.with_entity(|entity| {
            let pearl = entity.downcast_ref::<EnderPearl>().expect("ender pearl");
            assert_eq!(
    ...
    fn owner_death_vanish_predicate_respects_won_game() {
        assert!(EnderPearl::should_vanish_for_owner_state(
            false, false, true
        ));
        assert!(!EnderPearl::should_vanish_for_owner_state(
            false, true, true
