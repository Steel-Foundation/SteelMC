...
impl FireworkRocketEntity {
    ...
    pub fn attached_to_living(
        ...
        rocket.state.lock().attached_to_entity = Some(Arc::downgrade(attached_to));
    ...
    fn deal_explosion_damage(self: &Arc<Self>, world: &Arc<World>) {
        ...
        for target in world.get_entities_in_aabb_matching(&search_box, Entity::is_living_entity) {
            if attached
                .as_ref()
                .is_some_and(|attached| Arc::ptr_eq(attached, &target))
            {
                continue;
            }
...
mod tests {
    ...
    fn attached_rocket_keeps_its_target_when_world_ids_are_reused() {
        ...
        let replacement: SharedEntity = Arc::new(PigEntity::new(
            &vanilla_entities::PIG,
            target.id(),
            DVec3::ZERO,
            Arc::downgrade(&world),
    ...
    fn explosion_damages_replacement_with_attached_entity_id() {
        ...
        let replacement = Arc::new(PigEntity::new(
            &vanilla_entities::PIG,
            original.id(),
            position + DVec3::X,
            Arc::downgrade(&world),
