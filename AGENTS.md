...
The `minecraft-src/` folder should be generated by `./script/update-minecraft-src.sh` in bash or `./script/update-minecraft-src.ps1` in powershell and kept current with the target version from `Cargo.toml`.
If it isn't, stop and state the issue to the user.
