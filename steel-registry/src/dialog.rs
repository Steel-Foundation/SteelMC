...
impl RegistryExt for DialogRegistry {
    type Entry = DialogRef;

    fn freeze(&mut self) {
        self.allows_registering = false;
    }

    fn by_id(&self, id: usize) -> Option<DialogRef> {
        self.dialogs_by_id.get(id).copied()
    }

    fn by_key(&self, key: &Identifier) -> Option<DialogRef> {
        self.dialogs_by_key.get(key).and_then(|&id| self.by_id(id))
    }

    fn id_from_key(&self, key: &Identifier) -> Option<usize> {
        self.dialogs_by_key.get(key).copied()
    }
...
impl RegistryEntry for Dialog {
    ...
    fn try_id(&self) -> Option<usize> {
        REGISTRY.dialogs.id_from_key(&self.key)
    }
}
