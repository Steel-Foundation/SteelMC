...
/// Resolves tag references recursively and returns a flattened list of timeline keys
fn resolve_tag(
    tag_name: &str,
    ...
    for value in values {
        if let Some(nested_tag) = value.strip_prefix('#') {
            ...
            // Direct timeline reference - remove "minecraft:" prefix
            let timeline_key = value.strip_prefix("minecraft:").unwrap_or(value);
            resolved.push(timeline_key.to_string());
        }
    }
...
pub(crate) fn build() -> TokenStream {
    ...
    stream.extend(quote! {
        use crate::damage_type::DamageTypeRegistry;
        use steel_utils::Identifier;
    });

    let mut register_stream = TokenStream::new();
    // Generate const arrays for each tag
    for (tag_name, damage_type) in &sorted_tags {
        let tag_ident = Ident::new(
            &format!("{}_TAG", tag_name.to_shouty_snake_case()),
            Span::call_site(),
        );

        let damage_type_strs = damage_type.iter().map(|s| s.as_str());

        stream.extend(quote! {
            static #tag_ident: &[&str] = &[#(#damage_type_strs),*];
        });
    ...
    stream.extend(quote! {
        pub fn register_damage_types_tags(registry: &mut DamageTypeRegistry) {
            #register_stream
        }
    });
