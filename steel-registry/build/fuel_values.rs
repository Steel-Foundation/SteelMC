#![expect(clippy::unwrap_used, reason = "build script parses extracted vanilla data")]
use std::fs;
...
#[derive(Deserialize)]
struct Fuel { item: String, burn_time: i32 }

pub(crate) fn build() -> TokenStream {
    println!("cargo:rerun-if-changed=build_assets/fuels.json");
    let fuels: Vec<Fuel> = serde_json::from_str(&fs::read_to_string("build_assets/fuels.json").unwrap()).unwrap();
    let arms = fuels.iter().map(|fuel| {
        let key = fuel.item.strip_prefix("minecraft:").unwrap_or(&fuel.item);
        let ident = Ident::new(&key.to_uppercase().replace('-', "_"), proc_macro2::Span::call_site());
