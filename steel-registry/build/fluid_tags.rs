...
/// Resolves tag references recursively and returns a flattened list of fluid keys
fn resolve_tag(
    tag_name: &str,
    ...
    for value in values {
        if let Some(nested_tag) = value.strip_prefix('#') {
            ...
            // Direct fluid reference - remove "minecraft:" prefix
            let fluid_key = value.strip_prefix("minecraft:").unwrap_or(value);
            resolved.push(fluid_key.to_string());
        }
    }
...
pub(crate) fn build() -> TokenStream {
    ...
    for (tag_name, fluids) in &sorted_tags {
        ...
        stream.extend(quote! {
            static #tag_ident: &[&str] = &[#(#fluid_strs),*];
        });
